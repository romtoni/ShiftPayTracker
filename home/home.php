...
if ($_SESSION["server_role_sejati"]=="1")
{
	global $ganti_role, $msg;
	
	$ganti_role = $_REQUEST["ganti_role"];
	$kode_role = $ganti_role;
	
	if ($ganti_role=="Y") $msg = "Ganti Role Temporer Berhasil";
	else if ($ganti_role=="") $msg = "";
	else
	{
		$strsql="select count(*) as JUMLAH from MN_ROLE where kode_role='$kode_role'";
		//echo $strsql;
		...
		if ($jumlah==0) $msg = "Role Tidak Terdaftar";
		else
		{
...
{
	$userlogin = strtoupper($_SESSION["server_user"]);
	$msg="Hey $userlogin, Jangan macam-macam ya!";
}
?>
