...
function delete()
{
	...
	for($i=1; $i<=$nrow; $i++) 
	{
 		if (isset($_POST["cek$i"])) $kode_role=$_POST["cek$i"];
  		if($kode_role!="")
		{
			$strsql="BEGIN DELETE_INVALID_USER('$kode_role'); END;";
...
function open()
{
	...
	$strsql="select count(*) as JUMLAH from V_INVALID_USER";
	$q=oci_parse($conn,$strsql);
	oci_execute($q);
	...
	if ($jumlah==0)
	{
		...
		$datatmpl.="<td align='center' colspan='5'><h3>Tidak Ada Data</h3></td>";
		$datatmpl.="</tr>";
	}
	else
	{
		...
		while ($row=oci_fetch_assoc($q)) 
		{
			...
			$datatmpl.="<tr class='$class'>";
			$datatmpl.="<td align='center'><input type='checkbox' value='$kode_role' name='cek$no' id='cek$no'></td>";
			$datatmpl.="<td align='center'>$no</td>";
			$datatmpl.="<td align='center'>$kode_role</td>";
			$datatmpl.="<td align='center'>-NULL-</td>";
			$datatmpl.="<td align='center'>$username</td>";
			$datatmpl.="</tr>";
		}
		$datatmpl.="<tr>
           		<td align='left' colspan='4'>
           		 <input name='btnsubmit' id='btnsubmit' type='submit' class='button' value='Hapus User'>
				</td>
            </tr>";
	}
  ...
  <tr>
    ...
    <td width='960px'><table width="100%"  border="0" align="center" class="outside_border">
        ...
        <? require_once("../inc_menu.php"); ?>
                    <tr>
                      <td width="100%" align="center" class="title_banner"> INVALID USER</td>
                    </tr>
        <tr>
          ...
          <input type='hidden' name="nrow" id="nrow" value="<?=$jumlah?>">
            <table width="100%" border="0">
              <tr>
                ...
                <table width="500" height="111"  border="0" cellpadding="0" cellspacing="0">
                    <tr>
                      <td height="80" valign="top" bordercolor="#999999"><table width="100%" border="0" cellpadding="2" cellspacing="2">
                        <thead>
                        <tr class="table_header">
                          <td  width="8" align="center" bordercolor="#999999"><input name="cek_all" type="checkbox" id="cek_all" onClick="cek_semua();" value="Y"></td>
                          <td  width="19" align="center">No</td>
                          <td  width="84" align="center" >Kode Role</td>
                          <td  width="68" align="center" >Nama Role</td>
                          <td  width="68" align="center" >Nama User</td>
                        </tr>
                        </thead>
                        <tbody id="mytable">
                        	<?=$datatmpl?>
