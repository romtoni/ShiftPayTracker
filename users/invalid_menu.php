...
function delete()
{
	...
	for($i=1; $i<=$nrow; $i++) 
	{
 		if (isset($_POST["cek$i"])) $kode_menu=$_POST["cek$i"];
  		if($kode_menu!="")
		{
			$strsql="BEGIN DELETE_INVALID_MENU('$kode_menu'); END;";
			$q=oci_parse($conn,$strsql);
			oci_execute($q);
		}
	}
	header("location:invalid_menu.php");
}
...
function open()
{
	...
	$strsql="select count(*) as JUMLAH from
			(
				select 
					a.nama_menu,
					a.kode_menu 
	...
	if ($jumlah==0)
	{
		...
		$datatmpl.="<td align='center' colspan='4'><h3>Tidak Ada Data</h3></td>";
		$datatmpl.="</tr>";
	}
	else
	{
		...
		while ($row=oci_fetch_assoc($q)) 
		{
			...
			$datatmpl.="<tr>";
			$datatmpl.="<td align='center'><input type='checkbox' value='$kode_menu' name='cek$no' id='cek$no'></td>";
			$datatmpl.="<td align='center'>$no</td>";
			$datatmpl.="<td align='center'>$kode_menu</td>";
			$datatmpl.="<td align='center'>-NULL-</td>";
			$datatmpl.="</tr>";
		}
		$datatmpl.="<tr>
           		<td align='left' colspan='4'>
           		 <input name='btnsubmit' id='btnsubmit' type='submit' class='button' value='Hapus Menu'>
				</td>
            </tr>";
	}
  ...
  <tr>
    ...
    <td width='960px'><table width="100%"  border="0" align="center" class="outside_border">
        ...
        <? require_once("../inc_menu.php"); ?>
                    <tr>
                      <td width="100%" align="center" class="title_banner"> INVALID MENU</td>
                    </tr>
        <tr>
                    ...
                    <tr>
                      <td height="80" valign="top" bordercolor="#999999"><table width="100%" border="0" cellpadding="2" cellspacing="2">
                        ...
                        <tr class="table_header">
                          <td  width="23" align="center" bordercolor="#999999"><input name="cek_all" type="checkbox" id="cek_all" onClick="cek_semua();" value="Y"></td>
                          <td  width="46" align="center">No</td>
                          <td  width="77" align="center" >Kode Menu</td>
                          <td  width="256" align="center" >Nama Menu</td>
                        </tr>
                        </thead>
                        <tbody id="mytable">
                        	<?=$datatmpl?>
